...
const model = require('./target_type.model');
...
/**
 * @swagger
 * /administration/target-type:
 *   get:
 *     tags:
 *       - administration-target-type
 *     description: target-type administration
 *     parameters:
 *       - name : X-TOKEN
 *         type : string
 *         in: header
...
router.get('/',
    isAuthenticate,
    hasPermission(ADMIN_RESOURCES.TARGET_TYPE, ACTIONS.READ),
    checkValidations("target-type_gets"),
    catchAsync(async (req, res) => {
        res.json(setResponse(await model.gets(req.query)));
    }));
...
router.post('/',
    isAuthenticate,
    hasPermission(ADMIN_RESOURCES.TARGET_TYPE, ACTIONS.CREATE),
    checkValidations("target-type_create"),
    catchAsync(async (req, res) => {
 ...
 *               {"code":"2","result":"status is empty","is_login":"0"}
 *               {"code":"3","result":"status is not valid","is_login":"0"}
 *               {"code":"1","result":"target-type not found","is_login":"0"}
 *               </pre>
 *       404:
 *         description: not found
 *       500:
...
router.put('/:id',
    isAuthenticate,
    hasPermission(ADMIN_RESOURCES.TARGET_TYPE, ACTIONS.UPDATE),
    checkValidations("target-type_update"),
    catchAsync(async (req, res) => {
        const data = {id: req.params.id, target_type: req.body};
 ...
 *               {"code":"2","result":"id is empty","is_login":"0"}
 *               {"code":"3","result":"id is not valid","is_login":"0"}
 *               {"code":"1","result":"target_type not found","is_login":"0"}
 *               {"code":"20","result":"target_type used in program","is_login":"0"}
 *               </pre>
 *       404:
 *         description: not found
 *       500:
...
router.delete('/:id',
    isAuthenticate,
    hasPermission(ADMIN_RESOURCES.TARGET_TYPE, ACTIONS.DELETE),
    checkValidations("target-type_delete"),
    catchAsync(async (req, res) => {
...
/**
 * @swagger
 * /administration/target-type/select-list:
 *   get:
 *     tags:
 *       - administration-target-type
 *     description: select-list target-type administration
 *     parameters:
 *       - name : X-TOKEN
 *         type : string
 *         in: header
